---
title: "Ensemble_Methods_for_Outlier_Detection_Version_1.9"
authors: "Jian Song, Nathan P. Manes"
date: "5/2/2022"
...
## 5. Identification of Outlier(s)

In this analysis, we used three different methods to exam the samples. They complement each other in identifying potential outlier(s). First, the density plot show how simililar in the expression pattern across the genome/proteome among the samples; Second, the hiearchical clustering created using distance matrix also show how close is each sample to each other while closely related samples form a tighter cluster. Cophenetic Correlation Coefficient (CCC) is used to evaluate whether the dendrogram is not a good representation of the relationships among the samples and Silhouette coefficient (SC) is used to show whether a sample belongs to a cluster; and finally, robust PCA anlyses performed by two different methods, robpca and PcaGrid, were used to identify the outlier(s). The PcaGrid test was described by Chen et al 2020, and the CCC, SC, and robpca tests are by Selicato et al 2021.

While the Robust PCA analyses are able to "robustly" identify the outlier(s) and explain the types of outlier(s), density plot and hiearchical clustering additional visual confirmation by showing the outlier(s) are different and further distant from rest of the samples.

### 5.1 Outlier(s) I dentified by Robust PCA analyses
