...
```{r getData, warning=FALSE, message=TRUE, fig.width=12, fig.height=6}
...
# For normally distributed data, this value is the estimated fraction of the samples that are not falsely classified as outliers.
PcaGrid_prob <- as.numeric(params$PcaGrid_prob)
PcaGrid_prob
...
input_data <- readxl::read_xlsx(toString(params$inputFile), skip = 0, col_names = TRUE, col_types=NULL)
...
# Also save a copy as PDF 
pdf(file="./www/EnsMODoutputs/Density_plot.pdf")
plotDensities(input_data_nna, main="Densities of the Abundance Values for each Sample", legend = "topright") 
dev.off()
...
input_data_nna_ModZ_0 <- matrix(nrow = input_data_nna_rows, ncol = input_data_col)
for (i in 1:input_data_nna_rows) {
 for (j in 1:input_data_col) {
   input_data_nna_ModZ_0[i,j] <- 0.6745 * (input_data_nna[i,j] - input_data_nna_median[i]) / input_data_nna_MAD[i]
 }
}

# To use regular Z-scores, enable the line below. This can be used to avoid artifacts from using the Modified Z-score.
# input_data_nna_ModZ_0 <- t(scale(t(input_data_nna), center = TRUE, scale = TRUE))

# Remove any row (genes/proteins) with one or more invalid values
input_data_nna_ModZ <- input_data_nna_ModZ_0[complete.cases(input_data_nna_ModZ_0), ]
...
hist_range_normal <- dnorm(hist_range, mean = 0, sd = 1)
...
density_y_model <- dnorm(density_x, mean = 0, sd = 1)
...
# Also save a copy as PDF
pdf(file="./www/EnsMODoutputs/Hierachical_clustering.pdf")
plot(hc.res_a)
dev.off()
...
# Display table of the non-robust PCA results (all PCs or just PC1 vs. PC2)
# PCA_classic$x
# PCA_classic$x[,c(1,2)]
...
# Display table of the robpca results (all PCs or just PC1 vs. PC2)
# resR0$scores
# resR0$scores[,c(1,2)]
...
# Also save it as a pdf file
pdf(file="./www/EnsMODoutputs/Robust_Sparse_PCA.pdf")
diagPlot(resR0, title = "Robust PCA (robpca)", pch = 17, col="red", labelOut = TRUE, id = 5)
dev.off()
...
```{r PcaGrid, warning=FALSE, message=TRUE, fig.width=8, fig.height=8}
library(rrcov)
...
# Display table of the PcaGrid results (all PCs or just PC1 vs. PC2)
# pc$scores
# pc$scores[,c(1,2)]
...
# Also save it as a pdf file
pdf(file="./www/EnsMODoutputs/PcaGrid.pdf")
diagPlot(pc, title = "Robust PCA (PcaGrid)", pch = 17, col="red", labelOut = TRUE, id = 5)
dev.off()
...
# Save the outlier to a file
write.csv(intersect(pcOutliers, rosOutliers), file="./www/EnsMODoutputs/Outliers_identified_by_both_RobustPCAs.csv")
```
<br>
...
write.csv(intersect(hcOutliers, rosOutliers), file="./www/EnsMODoutputs/Outliers_identified_by_HierarchicalClustering.csv")
...
# Save the outlier to a file
write.csv(intersect(intersect(hcOutliers, rosOutliers), pcOutliers), file="./www/EnsMODoutputs/Outliers_identified_by_EnsembleMethods.csv")
```
<br>
