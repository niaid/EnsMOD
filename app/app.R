...
list.of.packages <- c("igraph", "rlang", "Rcpp", "tidyverse", "shiny", "shinyjs", "xfun", "DT", "factoextra", "readr", "dplyr", "data.table", "reshape2", "htmltools", "readxl", "stats", "rrcov", "cluster", "ggraph", "RColorBrewer", "tidyverse", "gplots", "fitdistrplus")
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "CCC_min",
                  ...
                  choices = c(0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95),
                  selected = 0.8
      ),
      selectInput(inputId = "SC_max",
                  label = "Silhouette Coefficient (SC):",
                  choices = c(0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5),
                  selected = 0.25
      ),
...
server <- function(session, input, output){
  ...
  # Contact us by sending email to ensmod-team@list.nih.gov
